...
their packages need updating when a newer project is available. Many rmtools
programs are designed to help distributions add their packages to r-m.
...
program won't handle your package manager (it is currently tweaked for
RPM-based distros only), create the list some other way.  This list is used as
input in most of the remaining steps.  `metadata-parse` is actually seldom the
best way to generate the list because it won't write the package's source code
link.

2. Use `add-matching` to associate the low-hanging fruit (i.e. add these
packages to existing projects in r-m).  This program is quite safe to run (i.e.
it has a low likelihood of associating a package with the wrong project). It
relies on two bits of information supplied by you: the r-m project name and its
...
names fed to `add-matching` (but NOT from the *package* names) is likely to
allow it to find the correct project in r-m. Since the URL is still used for
...
"index.html" removed, etc.). This can result in many queries for each package
which is slow and inefficient, so this should be done only after as many
associations as possible have already been made by `add-matching`. The
`--dump-existing` option in `add-matching` will write a file containing all
known packages for a distribution already in r-m, which is useful for filtering
only those packages left to process.
...
through those. `create-project` has the `--strip-project-prefix` option to
...
going to ever see another update and isn't worth adding to r-m.

## Programs

### check-latest-versions

This program checks release-monitoring.org for version updates of one or more
...
Entries under `check_packages` and `check_packages_unstable` are the distro
names for which packages are to be checked. Under each distro list the packages
...
.spec files. Some amount of scripting on those files, likely based on
...
The `--distro` option sets the short name of the distribution as it will be
shown in r-m.
...
requires a match on another piece of metadata before a package is added. This
...
`--no-existence-check`, it will then dump only newly-added packages.
...
project package URL
```
containing with the matched information. This can be fed into `add-matching` to
add these packages to the given projects.  This shouldn't be done without first
double-checking the projects to ensure they are the correct ones. It often
happens that an umbrella URL (for an organization, for example) will be used
for many sub-projects in a site, so false positives can easily occur.
...
Only certain known source URLs are supported and any other project is skipped,
since the knowledge of how to properly check versions is only known for a few
